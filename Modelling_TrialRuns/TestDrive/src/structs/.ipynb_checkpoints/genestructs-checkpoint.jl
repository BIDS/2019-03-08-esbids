...
struct Drive{G <: Genotype} 
    ...
    τ::Vector{Float64}          #τ::Array{Float64,2}   # offspring viability
...
struct Genetics 
    ...
    S::Vector{Float64}                       # fractional reduction in fertility, per genotype - masks the number of eggs (scales the beta )
    Τ::Array{Float64,2}                      # Τ::Array{Float64,3}                      # offspring viability, per genotype  
    ...
    Η::Vector{Float64}                       # male mating fitness, per genotype 
        ...
        function Genetics(all_genotypes::Array{Drive{<:Genotype}}) 
            ...
            Τ = Array{Float64,2}(undef, gN, gN, gN) #Τ = Array{Float64,3}(undef, gN, gN, gN)
